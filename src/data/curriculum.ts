 ...
 *
 * 用途は2つある。
 *   1. 監査 ── 自己申告と実際がずれていないか（逆で覚えやすいところに置く）
 *   2. 初日の診断 ── `logic/placement.ts` が聞く観点。依存の深いところに要る
 * 診断は答え合わせできる観点からしか聞けないので、
 * `placement` が選ぶ場所に問いが無いと、そもそも診断が始まらない。
 *
...
export const CHECKS: Record<string, CheckQuestion> = {
  "m1-eq": {
    prompt: "−2x = 8 のとき、x は？",
    choices: ["−4", "4", "−16", "16"],
    answerIndex: 0, // 負の数で割るときに符号を落としやすい
  },
  "m1-circle-basic": {
    prompt: "半径6・中心角60°のおうぎ形の弧の長さは？",
    choices: ["2π", "6π", "π", "12π"],
    answerIndex: 0,
  },
  "m2-simul": {
    prompt: "x + y = 5、x − y = 1 の解は？",
    choices: ["x = 3, y = 2", "x = 2, y = 3", "x = 4, y = 1", "x = 1, y = 4"],
    answerIndex: 0, // x と y を入れ替えやすい
  },
  "m2-proof": {
    prompt: "証明で先に書くのは、仮定と結論のどちら？",
    choices: ["仮定", "結論", "どちらでもよい", "使う合同条件"],
    answerIndex: 0,
  },
  "m2-linear-find": {
    prompt: "2点 (0, 1) と (2, 5) を通る直線の式は？",
    choices: ["y = 2x + 1", "y = x + 1", "y = 2x + 5", "y = (1/2)x + 1"],
    answerIndex: 0, // 傾きを逆数にしやすい
  },
  "m3-factor": {
    prompt: "x² + 5x + 6 を因数分解すると？",
    choices: ["(x + 2)(x + 3)", "(x − 2)(x − 3)", "(x + 1)(x + 6)", "(x + 2)(x + 4)"],
    answerIndex: 0, // 符号をまとめて逆にしやすい
  },
  "m3-sqrt-calc": {
    prompt: "√2 × √8 の値は？",
    choices: ["4", "√10", "2√2", "16"],
    answerIndex: 0, // 根号の中でたし算にしやすい
  },
  "m3-similar": {
    prompt: "三角形の「相似」条件に入らないものはどれ？",
    choices: [
      "3組の辺がそれぞれ等しい",
      "2組の角がそれぞれ等しい",
      "3組の辺の比がすべて等しい",
    ...
    answerIndex: 0, // これは合同条件。相似と取り違えやすい
  },
  "m3-quadfn-range": {
    prompt: "y = x² で −1 ≦ x ≦ 2 のとき、y の変域は？",
    choices: ["0 ≦ y ≦ 4", "1 ≦ y ≦ 4", "−1 ≦ y ≦ 4", "0 ≦ y ≦ 2"],
    answerIndex: 0, // 両端だけ入れると 1 ≦ y ≦ 4 になる。0 をまたぐのを忘れやすい
  },
  "m1-prob-basic": {
    prompt: "回数を多くしたとき、相対度数が近づいていくのは？",
    choices: ["その事がらの確率", "かならず 0.5", "かならず 1", "度数の合計"],
    answerIndex: 0,
  },
  "m1-signed": {
    prompt: "−2² を計算すると？",
